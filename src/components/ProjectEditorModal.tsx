          ...
          <div className="pt-4 border-t-2 border-black flex items-center justify-between gap-3">
            {isEditing && onDelete ? (
              <button
                type="button"
                onClick={() => {
                  if (window.confirm(`Delete project "${title}" from archives?`)) {
                    onDelete(projectToEdit.id);
                    onClose();
                  }
                }}
                className="px-4 py-2 border-2 border-black bg-white hover:bg-red-600 hover:text-white text-xs font-mono font-bold flex items-center gap-1.5 transition-colors cursor-pointer"
              >
                <Trash2 className="w-3.5 h-3.5" />
